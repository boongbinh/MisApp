...
List<_ColDef> _buildColDefs(MaybomDetailCNKVViewModel vm) {
  return [
    if (vm.showDonVi.value) const _ColDef('donVi', 'Đơn vị', 0.6),
    if (vm.showKyHieu.value) const _ColDef('kyHieu', 'Ký hiệu', 1.4),
    if (vm.showChucNang.value) const _ColDef('chucNang', 'Chức năng', 1.6),
    if (vm.showNamSX.value) const _ColDef('namSX', 'Năm SX', 0.5),
    if (vm.showNamSD.value) const _ColDef('namSD', 'Năm SD', 0.5),
    if (vm.showNuocSX.value) const _ColDef('nuocSX', 'Nước SX', 0.6),
    if (vm.showHangSXBom.value) const _ColDef('hangSXBom', 'Hãng SX bơm', 0.9),
    if (vm.showModelBom.value) const _ColDef('modelBom', 'Model bơm', 0.9),
    if (vm.showKieuBom.value) const _ColDef('kieuBom', 'Kiểu bơm', 1.2),
    if (vm.showLuuLuong.value) const _ColDef('luuLuong', 'Lưu lượng', 0.8),
    if (vm.showChieuCaoDay.value) const _ColDef('chieuCaoDay', 'Chiều cao đẩy', 0.8),
    if (vm.showHangSXDongCo.value) const _ColDef('hangSXDongCo', 'Hãng SX động cơ', 1.0),
    if (vm.showModelDongCo.value) const _ColDef('modelDongCo', 'Model động cơ', 1.0),
    if (vm.showKieuDongCo.value) const _ColDef('kieuDongCo', 'Kiểu động cơ', 0.8),
    if (vm.showCongSuatDongCo.value) const _ColDef('congSuatDongCo', 'Công suất ĐC', 0.8),
    if (vm.showDienAp.value) const _ColDef('dienAp', 'Điện áp', 0.7),
    const _ColDef('settings', '', 0.3),
  ];
}

double _getTotalFlex() {
  // 0.6 + 1.4 + 1.6 + 0.5 + 0.5 + 0.6 + 0.9 + 0.9 + 1.2 + 0.8 + 0.8 + 1.0 + 1.0 + 0.8 + 0.8 + 0.7 + 0.3 = 13.4
  return 13.4;
}

class MaybomDetailCNKV extends GetView<MaybomDetailCNKVViewModel> {
  const MaybomDetailCNKV({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Tổng hợp máy bơm'),
        centerTitle: true,
        elevation: 0,
                ...
                children: [
                  ...
                  Expanded(
                    child: Container(
                      ...
                      child: Column(
                        children: [
                          // ⭐ Dùng GetBuilder cho header
                          GetBuilder<MaybomDetailCNKVViewModel>(
                            id: 'header',
...
class _SearchBar extends StatelessWidget {
  ...
  final MaybomDetailCNKVViewModel vm;
...
class _TableRow extends StatelessWidget {
  ...
  Widget _cell(String key, Map<String, dynamic> item) {
    final text = switch (key) {
      'donVi' => item['Don_vi']?.toString() ?? '--',
      'kyHieu' => item['Ma_bom']?.toString() ?? '--',
      'chucNang' => item['Chuc_nang']?.toString() ?? '--',
      'namSX' => item['Nam_sx']?.toString() ?? '--',
      'namSD' => item['Nam_sd']?.toString() ?? '--',
      'nuocSX' => item['Nuoc_sx']?.toString() ?? '--',
      'hangSXBom' => item['Hang_sx_bom']?.toString() ?? '--',
      'modelBom' => item['Model_bom']?.toString() ?? '--',
      'kieuBom' => item['Kieu_bom']?.toString() ?? '--',
      'luuLuong' => item['Luuluong']?.toString() ?? '--',
      'chieuCaoDay' => item['Chieucao_day']?.toString() ?? '--',
      'hangSXDongCo' => item['Hang_sanxuat']?.toString() ?? '--',
      'modelDongCo' => item['Model_dongco']?.toString() ?? '--',
      'kieuDongCo' => item['Kieu_dongco']?.toString() ?? '--',
      'congSuatDongCo' => item['Cong_suat_dongco']?.toString() ?? '--',
      'dienAp' => item['Dienap']?.toString() ?? '--',
      _ => '--',
    };
