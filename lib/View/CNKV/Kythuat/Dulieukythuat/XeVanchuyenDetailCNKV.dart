...
List<_ColDef> _buildColDefs(XeVanchuyenDetailCNKVViewModel vm) {
  return [
    const _ColDef('settings', '', 0.3),

    if (vm.showDoiVanTai.value) const _ColDef('doiVanTai', 'Đội vận tải', 0.8),
    if (vm.showBienKiemSoat.value) const _ColDef('bienKiemSoat', 'Biển kiểm soát', 1.0),
    if (vm.showLoaiXe.value) const _ColDef('loaiXe', 'Loại xe', 1.0),
    if (vm.showTenXe.value) const _ColDef('tenXe', 'Tên xe', 1.3),
    ...
    if (vm.showNamHetHanSD.value) const _ColDef('namHetHanSD', 'Năm hết hạn SD', 0.8),
    if (vm.showCKDK.value) const _ColDef('ckdk', 'CK đăng kiểm', 0.8),
    if (vm.showNgayKD.value) const _ColDef('ngayKD', 'Ngày đăng kiểm', 1.1),
    if (vm.showNgayKDTiep.value) const _ColDef('ngayKDTiep', 'Ngày ĐK kế tiếp', 1.1),
    if (vm.showDVDK.value) const _ColDef('dvdk', 'Đơn vị đăng kiểm', 1.6),
  ];
}

class XeVanchuyenDetailCNKV extends GetView<XeVanchuyenDetailCNKVViewModel> {
  const XeVanchuyenDetailCNKV({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Tổng hợp xe vận tải'),
        centerTitle: true,
        elevation: 0,
...
class _SearchBar extends StatelessWidget {
  ...
  final XeVanchuyenDetailCNKVViewModel vm;
                ...
                Container(
                  ...
                  child: defs[i].key == 'settings'
                      ...
                      : Text(
                          ...
                          maxLines: 2, // ⭐ Cho phép xuống dòng 2 line
