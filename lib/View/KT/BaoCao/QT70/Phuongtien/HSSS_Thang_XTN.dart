...
class HSSS_Thang_XTN extends GetView<HSSS_Thang_XTNViewModel> {
  const HSSS_Thang_XTN({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('THEO DÕI HSSS XE TRA NẠP'),
        centerTitle: true,
        elevation: 0,
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Obx(() {
                              ...
                              final groupedData = controller.getGroupedData();
                              return ListView(
                                children: groupedData.entries.map((entry) {
                                  final chinhanh = entry.key;
                                  final items = entry.value;
                                  final totals = controller.calculateTotals(items);
                                      ...
                                      Container(
                                        ...
                                        child: Row(
                                          children: [
                                            ...
                                            _BodyCell(
                                              controller.formatNumber(totals['Giothang']),
                                              flex: 1.0,
                                              fontWeight: FontWeight.bold,
                                            ),
                                            _BodyCell(
...
class _SearchAndFilterBar extends StatelessWidget {
  ...
  final HSSS_Thang_XTNViewModel vm;
...
class _FilterSheet extends StatefulWidget {
  static Future<void> open(BuildContext context, HSSS_Thang_XTNViewModel vm) async {
...
class _FilterSheetState extends State<_FilterSheet> {
  ...
  void initState() {
    ...
    tempMonth = vm.filterMonth.value;
  }
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: ComboField(
                    label: 'Tháng',
                    value: 'Tháng ${tempMonth.toString().padLeft(2, '0')}',
                    onTap: () => _pickMonth(context, initial: tempMonth, onSelected: (val) => setState(() => tempMonth = val)),
                  ),
                ),
              ],
            ),
  ...
  Future<void> _pickMonth(
    ...
    final sel = await _showListPicker<int>(
      ...
      title: 'Chọn tháng',
      items: months,
      display: (m) => 'Tháng ${m.toString().padLeft(2, '0')}',
      initial: months.indexOf(initial.clamp(1, 12)),
    );
    if (sel != null) onSelected(sel);
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _HeaderCell('GIỜ TRONG THÁNG', flex: 1.0),
