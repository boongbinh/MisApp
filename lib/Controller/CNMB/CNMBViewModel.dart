...
class CNMBModuleItem {
  final String title;
  final IconData icon;
  final bool enabled;
  final List<CNMBModuleItem> children; // ⭐ Thêm children cho menu con

  const CNMBModuleItem(
    this.title,
    this.icon, {
    this.enabled = true,
    this.children = const [],
...
class CNMBViewModel extends GetxController {
  final userName = ''.obs;
  final modules = <CNMBModuleItem>[].obs;
  ...
  void openModule(CNMBModuleItem m) {
    if (!m.enabled) return;

    if (m.title == 'Nhóm khai thác') {
      Get.toNamed(Routes.cnmbKhaithac);
    } else if (m.title == 'Nhóm kỹ thuật') {
      Get.toNamed(Routes.cnmbKythuat);
    } 
  }
  ...
  void _applyPermission(String perm) {
    ...
    // Kiểm tra từng quyền
    final hasNhomkhaithac = isAdmin || has("7.1. CNKVMB:Nhóm khai thác");
    final haskythuat = isAdmin || has("7.1. CNKVMB:Nhóm kỹ thuật");


    final list = <CNMBModuleItem>[
      CNMBModuleItem(
        "Nhóm khai thác",
