// components/Danhgiavapheduyet/danhgiabaocaongay_card.dart
import 'dart:math' as math;
import 'dart:ui' as ui;
import 'package:flutter/material.dart';
import 'package:intl/intl.dart';

class DanhgiabaocaongayData {
  final String label;
  final double value;
  final Color color;
  final String? suffix;
  
  DanhgiabaocaongayData({
    required this.label, 
    required this.value, 
    required this.color,
    this.suffix,
...
class DanhgiabaocaongayCard extends StatelessWidget {
  const DanhgiabaocaongayCard({
    super.key,
    required this.dataList,
    this.title = 'ĐÁNH GIÁ BÁO CÁO NGÀY',
    this.strokeWidth = 70,
    this.showPercents = true,
  });

  final List<DanhgiabaocaongayData> dataList;
  final String title;
  ...
  Widget build(BuildContext context) {
    // Lọc bỏ các item có value = 0
    final filteredData = dataList.where((item) => item.value > 0).toList();
    
    if (filteredData.isEmpty) return const SizedBox.shrink();

    final total = filteredData.fold(0.0, (sum, item) => sum + item.value);
    final segments = filteredData.map((e) {
