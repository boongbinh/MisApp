...
class ChiphitranapData {
  final String label;
  final double value;
  final Color color;
  ChiphitranapData({required this.label, required this.value, required this.color});
}

String _fmt2(num v) => NumberFormat('#,##0.##', 'vi_VN').format(v);
String _fmtTrieu(num v) => NumberFormat('#,##0.##', 'vi_VN').format(v / 1000000) + ' Triệu';

class ChiphitranapCard extends StatelessWidget {
  const ChiphitranapCard({
    super.key,
    required this.dataList,
    this.title = 'CHI PHÍ TRA NẠP THEO CHI NHÁNH',
    this.strokeWidth = 70,
    this.showPercents = true,
    this.showValueInMillion = true, // hiển thị giá trị dạng Triệu
  });

  final List<ChiphitranapData> dataList;
  final String title;
  final double strokeWidth;
  final bool showPercents;
  final bool showValueInMillion;
  ...
  Widget build(BuildContext context) {
    ...
    final totalText = showValueInMillion ? _fmt2(total) : _fmt2(total);
        ...
        children: [
          ...
          ...dataList.map((item) => Padding(
                ...
                child: _LegendRow(
                  ...
                  label: item.label,
                  value: item.value,
                  showValueInMillion: showValueInMillion,
                ),
              )),
        ],
      ),
...
class _LegendRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final displayValue = showValueInMillion ? _fmt2(value) : _fmt2(value);
    ...
    return Row(
      children: [
        ...
        Text(displayValue, style: const TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF1F2A37))),
