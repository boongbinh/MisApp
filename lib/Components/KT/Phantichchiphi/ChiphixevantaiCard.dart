...
class ChiphixevantaiData {
  final String label;
  final double value;
  final Color color;
  ChiphixevantaiData({required this.label, required this.value, required this.color});
}

String _fmt2(num v) => NumberFormat('#,##0.##', 'vi_VN').format(v);
String _fmtTrieu(num v) => NumberFormat('#,##0.##', 'vi_VN').format(v / 1000000) + ' Triệu';

class ChiphixevantaiCard extends StatelessWidget {
  const ChiphixevantaiCard({
    super.key,
    required this.dataList,
    this.title = 'CHI PHÍ XE VẬN TẢI THEO CHI NHÁNH',
    this.strokeWidth = 70,
    this.showPercents = true,
    this.showValueInMillion = true,
  });

  final List<ChiphixevantaiData> dataList;
  final String title;
