...
class BangBDSCvantaiRow {
  ...
  factory BangBDSCvantaiRow.fromMap(Map<String, dynamic> map) {
...
// ====== CONTROLLER ======
class BangBDSCvantaiController extends GetxController {
  final rows = <BangBDSCvantaiRow>[].obs;
  ...
  void loadData(List<Map<String, dynamic>> data) {
    rows.assignAll(data.map((e) => BangBDSCvantaiRow.fromMap(e)).toList());
  }
  ...
  String fmtKm(int? v) {
    if (v == null) return '--';
    // Chuyển từ m sang km (chia 1000)
    final km = v / 1000;
    return NumberFormat('#,###.##', 'vi_VN').format(km);
  }
...
// ====== WIDGET CHÍNH ======
class BangBDSCvantaiCard extends StatelessWidget {
  final List<Map<String, dynamic>> data;
  final String title;
  final String monthYear;

  const BangBDSCvantaiCard({
    super.key,
    required this.data,
    this.title = 'BẢNG BĐSC VẬN TẢI',
    this.monthYear = '',
  });

  @override
  Widget build(BuildContext context) {
    final controller = Get.put(BangBDSCvantaiController())..loadData(data);
...
class _TableHeader extends StatelessWidget {
  ...
  final BangBDSCvantaiController controller;
  final String monthYear;
  ...
  List<_ColDef> _buildColDefs(BangBDSCvantaiController c) {
    final defs = <_ColDef>[];
    ...
    if (c.showGioHDTT.value) {
      defs.add(const _ColDef(key: 'gioHDTT', title: 'Số km hoạt động', isTT: true));
    }
    ...
    if (c.showGioHDTK.value) {
      defs.add(const _ColDef(key: 'gioHDTK', title: 'Số km hoạt động', isTT: false));
    }
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                _switch('Số km hoạt động', controller.showGioHDTT),
                // Nhóm Thống kê
                const Padding(
                  padding: EdgeInsets.symmetric(vertical: 4),
                  child: Row(
                ...
                _switch('Số km hoạt động', controller.showGioHDTK),
                const SizedBox(height: 8),
                Row(
                  children: [
                    Expanded(
