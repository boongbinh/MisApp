...
class CocauDoanhthuData {
  final String label;
  final double value;
  final Color color;
  CocauDoanhthuData({required this.label, required this.value, required this.color});
}

String _fmt2(num v) => NumberFormat('#,##0.##', 'vi_VN').format(v);

class CocauDoanhthuCard extends StatelessWidget {
  const CocauDoanhthuCard({
    super.key,
    required this.dataList, // bắt buộc
    this.title = 'Cơ cấu doanh thu',
    this.strokeWidth = 70,
    this.showPercents = true,
  });

  final List<CocauDoanhthuData> dataList;
  final String title;
