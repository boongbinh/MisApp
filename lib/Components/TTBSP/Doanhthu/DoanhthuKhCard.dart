...
class DoanhthuKhData {
  final String label;
  final double value;
  final Color color;
  DoanhthuKhData({required this.label, required this.value, required this.color});
}

String _fmt2(num v) => NumberFormat('#,##0.##', 'vi_VN').format(v);

class DoanhthuKhCard extends StatelessWidget {
  const DoanhthuKhCard({
    super.key,
    required this.dataList, // bắt buộc
    this.title = 'Doanh thu theo khách hàng',
    this.strokeWidth = 70,
    this.showPercents = true,
  });

  final List<DoanhthuKhData> dataList;
  final String title;
