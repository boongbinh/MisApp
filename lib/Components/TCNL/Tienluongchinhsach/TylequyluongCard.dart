...
class TylequyluongData {
  final String label;
  final double value;
  final Color color;
  TylequyluongData({required this.label, required this.value, required this.color});
}

String _fmt(double v) {
  if (v == 0) return '0';
  return NumberFormat('#,##0.##', 'vi_VN').format(v / 1000000);
}


class TylequyluongCard extends StatelessWidget {
  const TylequyluongCard({
    super.key,
    required this.dataList,
    this.title = 'TỶ LỆ QUỸ LƯƠNG',
    this.strokeWidth = 70,
    this.showPercents = true,
  });

  final List<TylequyluongData> dataList;
  final String title;
  ...
  Widget build(BuildContext context) {
    ...
    final totalText = _fmt(total) + ' Triệu VND';
      ...
      builder: (_, __) {
        return CustomPaint(
          ...
          child: Center(
            child: LayoutBuilder(
              builder: (_, c) {
                final size = math.max(c.maxWidth, c.maxHeight);
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        ...
        Text(_fmt(value) + ' Triệu VND', style: const TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF1F2A37))),
