...
class PhanLoaiData {
  ...
  final double value;
  final Color color;
  PhanLoaiData({required this.label, required this.value, required this.color});
}

String _fmt2(num v) => NumberFormat('#,##0.##', 'vi_VN').format(v);

class PhanLoaiCqCard extends StatelessWidget {
  const PhanLoaiCqCard({
    super.key,
    required this.dataList, // bắt buộc
    this.title = 'Phân loại CQ',
    this.strokeWidth = 70,
    this.showPercents = true,
  });

  final List<PhanLoaiData> dataList;
  final String title;
